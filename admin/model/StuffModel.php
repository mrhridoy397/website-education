...
class StuffModel extends Db
{
    ...
    public function create(array $data): array
    {
        ...
        $this->query("INSERT INTO `stuff`( `stuffId`, `name`, `fname`, `mname`, `gender`, `dob`, `blood`, `relagion`,
         `isMarrid`, `phone`, `nid`, `education`, `subject`, `desigmation`, `department`, `salary`, `contactType`,
          `workShift`, `expriance`, `joiningDate`, `link1`, `link2`, `link3`, `link4`, `address`, `status`, `image`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        $this->bind(13, $data['subject']);
        $this->bind(14, $data['desigmation']);
    ...
    public function Update(array $data): array
    {
        ...
        $this->query("UPDATE `stuff` SET `stuffId`=?,`name`=?,`fname`=?,`mname`=?,`gender`=?,`dob`=?,`blood`=?,`relagion`=?,`isMarrid`=?,`phone`=?,`nid`=?,`education`=?,`subject`=?,`desigmation`=?,`department`=?,`salary`=?,`contactType`=?,`workShift`=?,`expriance`=?,`joiningDate`=?,`link1`=?,`link2`=?,`link3`=?,`link4`=?,`address`=?,`status`=?,`image`=? WHERE `id`=?");
        $this->bind(1, $data['stuffId']);
        $this->bind(2, $data['name']);
        $this->bind(3, $data['fname']);
        $this->bind(4, $data['mname']);
